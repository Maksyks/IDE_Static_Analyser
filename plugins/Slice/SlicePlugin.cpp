...
SliceResult SlicePlugin::runSlice(const QString& input, const QString& criterion) const {
    ...
    QByteArray out = proc.readAll();   // тут уже и stdout, и stderr
    ...
    // Разбираем bundle в структурированный SliceResult
    ...
    const QString bundle = QString::fromUtf8(result);
    qDebug()<<out;
    SliceResult r;
    r.custom     = between(out, "/*__BEGIN_ORIGINAL_CUSTOM__*/", "/*__END_ORIGINAL_CUSTOM__*/");
    r.llvm2c     = between(out, "/*__BEGIN_LLVM2C__*/",          "/*__END_LLVM2C__*/");
    const QString dotVis = between(bundle, "/*__BEGIN_DOT_VISIBLE__*/","/*__END_DOT_VISIBLE__*/");
    const QString dotCfg = between(bundle, "/*__BEGIN_DOT_CFG__*/",    "/*__END_DOT_CFG__*/");
    r.dotVisible = dotVis; r.dotCfg = dotCfg;
    qDebug() << "Получен бандл";
    qDebug() << r.custom;
    // lineMap парсим из JSON-мета, но НЕ оставляем в тексте
    const QString meta = between(out, "/*__IDE_METADATA__", "__IDE_METADATA_END__*/");
